...
class Controls {
    ...
    static setTerraceValueControls(): void {
        ...
        const terraceFenceHeight = <HTMLInputElement>document.getElementById("fenceHeight");
...
class PatioBuilder {
    ...
    static buildPatio() {
        ...
        if (PatioBuilder.patioElements) PatioBuilder.patioElements.forEach(e => TerraceDesigner.scene.remove(e));
        for (let i = 0; i < this.patioWidthCount; i++) {
            for (let d = 0; d < this.patioDepthCount; d++) {
                const x = i * this.patioBaseWidth - this.patioWidth / 2 + this.patioBaseWidth / 2,
                    y = d * this.patioBaseWidth - this.patioDepth / 2 + this.patioBaseWidth / 2,
    ...
    private static fences(): void {
        ...
        for (let i = 0; i < this.patioDepthCount; i++) {
            const x = this.patioWidth / 2 - this.fenceDepth / 2,
                y = i * this.patioBaseWidth - this.patioDepth / 2 + this.patioBaseWidth / 2,
                terraceFence = this.fence(x, y);
            TerraceDesigner.scene.add(terraceFence);
        }
        //The side fences
        for (let i = 0; i < this.patioWidthCount; i++) {
            const x = i * this.patioBaseWidth - this.patioWidth / 2 + this.patioBaseWidth / 2,
                yLeft = this.patioDepth / 2 - this.fenceDepth / 2,
                yRight = -(this.patioDepth / 2 - this.fenceDepth / 2),
                terraceFenceLeft = this.fence(x, yLeft, 90),
                terraceFenceRight = this.fence(x, yRight, 90);
    ...
    private static canopy(): void {
        ...
        const x = this.patioWidth / 2 - this.poleWidth / 2,
            y = this.patioDepth / 2 - this.poleWidth / 2,
            frontLeft = this.pole(x, y, this.poleHeight),
            frontRight = this.pole(x, -y, this.poleHeight),
            backLeft = this.pole(-x, y, this.poleHeight + 100),
            backRight = this.pole(-x, -y, this.poleHeight + 100),
            horizontalBarLeft = this.ceilingPole(x + this.poleWidth / 2, y, this.patioWidth, -90),
            horizontalBarRight = this.ceilingPole(x + this.poleWidth / 2, -y, this.patioWidth, -90),
            horizontalBarFront = this.ceilingPole(x, -y - this.poleWidth * 1.5, this.patioDepth + this.poleWidth * 2),
            horizontalBarBackTop = this.ceilingPole(-x, -y - this.poleWidth * 1.5, this.patioDepth + this.poleWidth * 2, 0, 100);
    ...
    private static fence(x: number, y: number, rot?: number): THREE.Mesh {
